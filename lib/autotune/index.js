...
function tuneAllTheThings (inputs) {
    ...
    if (pumpBasalProfile && pumpBasalProfile[0]) {
        for (hour=0; hour < 24; hour++) {
            ...
            newHourlyBasalProfile[hour].rate = Math.round(newHourlyBasalProfile[hour].rate*20)/20;
        }
    }
    ...
    for (hour=0; hour < 24; hour++) {
        if (hourlyBasalProfile[hour].rate === newHourlyBasalProfile[hour].rate) {
            ...
            newHourlyBasalProfile[hour].rate = Math.round( (0.8*hourlyBasalProfile[hour].rate + 0.1*newHourlyBasalProfile[lastAdjustedHour].rate + 0.1*newHourlyBasalProfile[nextAdjustedHour].rate)*20 )/20;
    ...
    newISF = Math.round( newISF * 10 ) / 10;
